...
class JoinContactNetworkImplTest {
    private lateinit var useCase: JoinContactNetworkImpl

    @MockK
    private lateinit var contactNetworkRepository: ContactNetworkRepository

    private val request = JoinContactNetwork.Request(user, contactNetwork)
    ...
    fun setUp() {
        ...
        useCase = JoinContactNetworkImpl(contactNetworkRepository)
    }
    ...
    fun `when there is no internet then the use case fails`() =
        runNoInternetTest(useCase, request) {
            coEvery {
                contactNetworkRepository.joinContactNetwork(any(), any())
    ...
    fun `when there is not any error then the use case succeeds`() =
        runSuccessTest(useCase, request) { response ->
            assertThat(response.contactNetworkName, isEqualTo(contactNetwork.name))
        }
}
